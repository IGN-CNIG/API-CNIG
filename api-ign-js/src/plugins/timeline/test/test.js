...
// const terremotosText = new M.layer.WMS('WMS*Eventos sísmicos*https://www.ign.es/wms-inspire/geofisica*NZ.ObservedEvent'); // ERROR si se hace así, se pone a este layer el id 2 luego 3 y al final termina con 4. Directamente usar ID 3 ahora da error ya que no existe, el grupo sigue funcionando bien porque coge el id 4 porque tiene en memoria la referencia al 2 que fue modificado cada vez.
// map.addLayers([geoJSON]); // El Plugin no tiene error si el layer ya estaba presente, la selección de grupo escoge y quita layers sin problemas.
...
// 1 - ERROR, No podía empezar las pruebas de este plugin por la falta de '"eslint-config-airbnb": "^16.1.0",' en devDependencies de "timeline/package.json".
// A la vez se observo que el archivo ".eslintrc" actual parece ser anticuado por lo que podría ser necesario actualizarlo y hacer todos los arreglos de este eslint.

// 2 - ERROR, parece que no se ha añadido traducción a este apartado "this.changeValueInputDate('Please choose an option', 'Please choose an option');" es siempre en ingles, podría ser mejor idea ponerlo en placeholder en vez de valor y dejar la opción vacía en esos casos de antes que lo ponían.
// O se podría dejar otro placeholder par explicar el formato esperado del tiempo, o texto de ejemplo al lado de estas opciones de input.
// También podría ser buena idea quitar la opción "0" inicial ya que realmente no se usa y así no hay que modificar nada más.
// Se ha podido evitar esos ceros con "this.date = { init: 'Please choose an option', end: 'Please choose an option' };" ya que como default se ponen a ceros que realmente son sobrantes.

// 3 - ERROR, la función "transformToLayers(layer) {..." de "timeline/src/facade/js/timelinecontrol.js" esta diseñada para devolver un return, pero no lo tiene.
// Con añadirle al final "return newLayer;" o "return null;" o "return undefined;" debería de funcionar igual posiblemente.
// Mientras que en "stepsTimeLineDinamic() {..." ocurre lo contrario, posiblemente sea buena idea añadir que tiene return. Se puede apuntar en descripción "* @return" y en el primer return de "return;" hay que ponerlo cono "return undefined;" para que no interprete este como una parada si no como return vacío.
// Hay tambien estas dos funciones "const filter = new M.filter.Function((f) => {..." que tienen que devolver al final "return false;" o "return undefined;" ya que el caso de que nunca devuelve nada no se ha previsto para evitar este error.

// 4 - ERROR, Hay muchos errores de "parseInt(val, radix)" que no tiene el segundo parámetro de radix, que posiblemente con un 10 es suficiente. Se podría hasta hacerse solo una vez para tener ya la variable para no llamarlo tantas veces. También esta "document.querySelector('.div-m-timeline-slider').style" y "document.querySelector('.m-timeline-names')" que se podrían reducir código con guardado de estos de ante mano. Aquí también en "changeSlider" se puede cambiar "document" por "this.template".
// También el uso de "isNaN" tiene que ser "Number.isNaN"

// 5 - ERROR, "const lastGroup_init" y "const lastGroup_end" no tienen el nombrado con formato "camelCase", se puede cambiar sin más a "lastGroupInit" y "lastGroupEnd" en todos sus usos.

// 6 - ERROR, No se distingue bien el estado de "Play/Pausa", podría ser mejor idea poner solo el que se va a usar en cada momento, la traducción de este y ademas de " Sin Capa " no existe. O se podría añadir a la clase "active" un estilo único para que se sepa que esta activado ahora.
// Al llegar al último elemento se debería de ponerse por si solo en pausa esta opción dejando el texto de "Play". Se podría hacer con tal código en "evtFormatMove":
// Primer crear un booleano de stop con default false "let stop = false;"
// Luego dentro del "if (id === 'm-timelineDinamic-before') {..." añadir "stop = sliderMax === numberIntEnd;" para asignar este.
// Y Finalizar con este if al terminar el ".map" del anterior cambio de asignado de stop.
// if (stop) {
//   const target = this.template.querySelector('#m-timelineDinamic-play');
...
// 7 - ERROR, Si se introduce una cadena de time por ejemplo "1716286905468" el convertidor "getValueDateInput" falla en loop en cada prueba de este, si se quiere usar este formato se puede hacer un parse en ese caso si se detecta "new Date(Number.parseInt(end)).getTime();"
// Porque al introducir los parámetros del plugin este formato si es valido.

// 8 - ERROR, 'this.intervals = this.intervals.filter(({ layer }) => {...' esta solo diseñado para el caso de objetos de intervals, en caso de ser arrays esto "{ layer }" tendría que referenciar al [2].
// Dejándolo de la siguiente forma:
// this.intervals = this.intervals.filter((inter) => {
//   let layer;
//   if (M.utils.isArray(inter)) {
//     layer = inter[2];
//   } else if (M.utils.isObject(inter) && inter.layer) {
//     layer = inter.layer;
//   } else return false;
...
// });
// A la vez este código es bastante confuso, porque "return layer.type !== 'GenericRaster' || layer.type !== 'GenericVector';" siempre devolverá true, porque no se puede confirmar que el type sea a la vez esas dos cadenas, mientras que el apartado de typeof layer === 'string' solo devuelve false si se pone en el mismo string los dos apartados "GenericRasterGenericVector"

// 9 - ERROR, En el formato de solo linea de tiempo, el final de los años no se pone bien con el final del input. Se ha comprobado que cambiando el ".div-m-timeline-slider .slider-tags { ... margin: -7px 13px 11px 13px; ... }" Se solucionó.

// 10 - ERROR, Se podría simplificar estas funciones con estos cambios directamente:
// getValueBeforeEvent(number, sliderMax) {
//   const calc = number + this.getStepValue();
//   return calc >= sliderMax ? sliderMax : calc;
// }
// getValueBackEvent(value, sliderMin) {
//   const calc = value - this.getStepValue();
//   return calc <= sliderMin ? sliderMin : calc;
// }
// getStepValue() {
//   switch (this.paramsDate) {
//     case 'sec':
//       return 1000 * this.stepValue;
//     case 'min':
//       return 60000 * this.stepValue; // (1000 * 60)
//     case 'hrs':
//       return 3600000 * this.stepValue; // (1000 * 60 * 60)
//     case 'day':
//       return 86400000 * this.stepValue; // (1000 * 60 * 60 * 24)
//     case 'mos':
//       return 2592000000 * this.stepValue; // (1000 * 60 * 60 * 24 * 30)
//     case 'yr':
//       return 31104000000 * this.stepValue; // (1000 * 60 * 60 * 24 * 30 * 12)
//     default:
//       return 86400000; // day in ms
//   }
// }
// getValueSliderB() {
//   const aux = this.template.querySelector('#b');
//   return {
//     min: Number(aux.min),
//     max: Number(aux.max),
//     value: Number(aux.value),
//   };
// }
// getValueSliderA() {
//   const aux = this.template.querySelector('#a');
//   return {
//     min: Number(aux.min),
//     max: Number(aux.max),
//     value: Number(aux.value),
...
//   if (format === 'logarithmic') return 'exponential';
//   if (format === 'exponential') return 'logarithmic';
//   return 'linear';
// }
// changeValueSlider(init, end) {
//   this.template.querySelector('.wrap input#a').value = init;
//   this.template.querySelector('.wrap input#b').value = end;
// }

// 11 - ERROR, El README tiene ciertos errores, el parámetro "timelineType" es obligatorio, por lo que el ejemplo de "absoluteSimple" al no tenerlo dará error, luego el parámetro "animation" solamente se usa en este mismo "timelineType: 'absoluteSimple'", pero no hay indicación de que se puede usar en este texto.

// 12 - ERROR, Se ha observado que el "timelineType: 'absolute'" no muestra el layer en la mayoría de casos, por ejemplo solo al poner 1975-01-01 se puede ver una de las capas, pero si no es exacto no se ve nada.

// 13 - ERROR, al poner 'relative' hay dos inputs para la lines del slider, el inicio y fin, el fin nunca puede ser inferior al inicio, pero la prioridad de click es siempre en fin ya que fue añadido más tarde con un z-index superior.
// Por lo que hay esta situación, si se lleva el input fin al final, no se puede moverlo atrás porque en caso de "timelineType: 'absolute'" el mínimo lo acompaña, ya que no se puede interactuar con este no se puede mover. Con poner z-index al 2 en #b es suficiente, pero el caso de que estén ambos se tendría que aumentar el z-index de #a de inicio por encima del z- index final, en este caso un 3. 

// 14 - ERROR JSP, directamente no puede añadir el plugin, porque el primer layer no esta definido. Parece que esta diseñado para aplicar el intervals con "Parámetro intervals" o "Capas disponibles", según que valor se escoge en "Tipo TimeLine", el problema es que al no haber default aplicados se sufre error en "return layer.type !== 'GenericRaster' || layer.type !== 'GenericVector';" porque layer es undefined y no puede tener type. El "8 - ERROR" explicaba el arreglo de este mismo apartado que da error.

// 15 - ERROR al usar "getAPIRest()" salta "Uncaught TypeError: cyclic object value", concretamente ocurre porque se puede poner layers enteros, y al haber dentro de estos referencias al map o otros apartados cíclicos este JSON.stringify falla

// 16 - ERROR, Este plugin no parece tener "getAPIRestBase64()" igual que los demás, no se si esto es un error o comportamiento esperado.

// 17 - ERROR, "Uncaught RangeError: invalid date" en "changeDateLayer timelinecontrol.js:694" si se envía una cadena vacía al input de fechas o cualquier otro texto no tratable.
