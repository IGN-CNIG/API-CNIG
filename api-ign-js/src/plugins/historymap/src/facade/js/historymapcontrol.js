/**
 * @module M/control/HistoryMapControl
 */

import HistoryMapControlImplControl from '../../impl/ol/js/historymapcontrol';
import template from '../../templates/historymap';

// import { getValue } from './i18n/language';


export default class HistoryMapControl extends M.Control {
  /**
   * @constructor
   * @extends {M.Control}
   * @api
  ...
  constructor(content = {}, delay = 2000, indexInContent) {
    const impl = new HistoryMapControlImplControl();
    super(impl, 'HistoryMapControl');
  ...
  createContent(allhtml, indexInContent) {
    const contentHistory = allhtml.querySelector('#contentHistoryMap');
    ...
    const title = allhtml.querySelector('#title_contentHistoryMap');
    title.innerHTML = cap[0].title;

    const subtitle = allhtml.querySelector('#subtitle_contentHistoryMap');
  ...
  disableCap(indexNextCap, indexCap, positionStep) {
    const divElement = document.querySelector('#contentHistoryMap');
  ...
  changeTitleSubtitle(indexNextCap) {
    const { cap } = this.content_;
    const title = document.querySelector('#title_contentHistoryMap');
    title.innerHTML = cap[indexNextCap].title;

    const subtitle = document.querySelector('#subtitle_contentHistoryMap');
    ...
    pointers.forEach((pointer, i) => {
      if (i !== 0 && i !== 1) {
        pointer.addEventListener('click', ({ target }) => {
          this.resetScroll();
          this.disableCap(target.getAttribute('index'), this.capIndex('#contentHistoryMap', '.chapters'), true);
  ...
  addJSCap(indexCap, indexStep) {
    try {
      if (document.querySelector('#historyMap_jsCap')) document.body.removeChild(document.querySelector('#historyMap_jsCap'));
      const newScript = document.createElement('script');
      newScript.id = 'historyMap_jsCap';
  ...
  timeCap() {
    const id = setInterval(() => {
      const idStep = this.capIndex(`#cap${this.capIndex('#contentHistoryMap', '.chapters')}`, '.step');
      const div = document.querySelector(`#cap${this.capIndex('#contentHistoryMap', '.chapters')}`);
      ...
      if (`#cap${this.capIndex('#contentHistoryMap', '.chapters')}` !== `#cap${lenghtCap}`
  ...
  getActivationButton(html) {
    return html.querySelector('.m-historymap button');
  }
   ...
   * @param {M.Control} control to compare
    ...
    */
  equals(control) {
    return control instanceof HistoryMapControl;
  }
}
