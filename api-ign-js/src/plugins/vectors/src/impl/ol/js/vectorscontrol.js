 ...
 export default class VectorsControl extends M.impl.Control {
   ...
   gmlParser(source) {
     ...
     geometriasPoligonos.forEach((geometria) => {
       ...
       for (let i = 0; i < geometria.vertices.length; i += 2) {
         polygonCoords.push(ol.proj.transform([parseFloat(geometria.vertices[i]), parseFloat(geometria.vertices[i + 1])], geometria.sistema, 'EPSG:4326'));
       }
   ...
   parseSurfacesGml(source) {
     ...
     while (auxSource !== '') {
       ...
       if (firstSurfaceTag > lastSurfaceTag) {
