...
export default class PredefinedZoomControl extends M.Control {
   ...
   */
  active(html) {
    const predefinedzoomactive = html.querySelector('#m-printviewmanagement-predefinedzoom').classList.contains('activated');
    this.deactive(html);
    if (!predefinedzoomactive) {
      if (M.utils.isArray(this.savedZooms_) && this.savedZooms_.length > 1) {
        ...
        html.querySelector('#m-printviewmanagement-predefinedzoom').classList.add('activated');
        ...
        this.savedZooms_.forEach((customZoom) => {
          ...
          newBtn.setAttribute('class', 'printviewmanagement-icon-expand');
        ...
        document.querySelector('#div-contenedor-printviewmanagement').appendChild(panel);
      } else {
        // predefinedZoom: true or array with one zoom level
        const element = html.querySelector('#m-printviewmanagement-predefinedzoom');
   ...
   */
  deactive(html) {
    html.querySelector('#m-printviewmanagement-predefinedzoom').classList.remove('activated');
    const panel = html.querySelector('#m-predefinedzoom-panel');
    if (panel) {
      document.querySelector('#div-contenedor-printviewmanagement').removeChild(panel);
    }
  }
