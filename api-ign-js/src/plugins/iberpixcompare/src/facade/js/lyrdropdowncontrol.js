  ...
  createView(map) {
    ...
    return new Promise((success, fail) => {
      ...
      this.template.querySelector('#m-lyrdropdown-selector').addEventListener('change', (evt) => {
        ...
        const layerActivated = this.fullLayers.filter((l) => {
          return l[2].indexOf(this.layerSelected.name) > -1;
        })[0];

        M.remote.get(layerActivated[3]).then((response) => {
          const geojson = JSON.parse(response.text);
          let intersecting = false;
          geojson.features.forEach((f) => {
            let geom = undefined;
            if (f.geometry.type === 'Polygon') {
              geom = new ol.geom.Polygon(f.geometry.coordinates);
            } else if (f.geometry.type === 'MultiPolygon') {
              geom = new ol.geom.Polygon(f.geometry.coordinates);
            }

            geom = geom.transform('EPSG:4326', map.getProjection().code);
          ...
          if (intersecting) {
            ...
            M.dialog.error(getValue('no_coverage'), getValue('warning'));
          }
        }).catch((error) => {
          this.layerSelected = layerSel[0];
          this.getImpl().setLayer(this.layerSelected);
