...
export default class LayerswitcherControl extends M.Control {
  ...
  clickLayer(evtParameter) {
    ...
    let notRender = false;
    if (!M.utils.isNullOrEmpty(evt.target)) {
      if (document.querySelector('.m-panel.m-plugin-layerswitcher.opened ul.m-layers') !== null) {
        scroll = document.querySelector('.m-panel.m-plugin-layerswitcher.opened ul.m-layers').scrollTop;
      }
      ...
      if (!M.utils.isNullOrEmpty(layerName) && layerURL !== null) {
        ...
        if (evt.target.classList.contains('m-check')) {
        ...
        } else if (evt.target.classList.contains('m-layerswitcher-zoom')) {
        ...
        } else if (evt.target.classList.contains('m-layerswitcher-legend')) {
          ...
          notRender = true;
        } else if (evt.target.classList.contains('m-layerswitcher-config')) {
          let otherStyles;
          ...
          setTimeout(() => {
            const selector = 'div.m-mapea-container div.m-dialog #m-layerswitcher-change-config button';
            ...
            this.accessibilityTab(document.querySelector('#m-layerswitcher-change-config'));
          }, 10);
        } else if (evt.target.classList.contains('m-layerswitcher-info')) {
          if (layer.type === 'OGCAPIFeatures') {
      ...
      } else if (evt.target.classList.contains('m-layerswitcher-addservice')) {
        ...
        setTimeout(() => {
          if (document.querySelector('#m-layerswitcher-addservices-list-btn') !== null) {
            document.querySelector('#m-layerswitcher-addservices-list-btn').addEventListener('click', e => this.showSuggestions(e));
            document.querySelector('#m-layerswitcher-addservices-list-btn').addEventListener('keydown', e => (e.keyCode === 13) && this.showSuggestions(e));
          }

          if (document.querySelector('#m-layerswitcher-addservices-codsi-btn') !== null) {
            document.querySelector('#m-layerswitcher-addservices-codsi-btn').addEventListener('click', e => this.showCODSI(e));
            document.querySelector('#m-layerswitcher-addservices-codsi-btn').addEventListener('keydown', e => (e.keyCode === 13) && this.showCODSI(e));
            document.querySelector('#m-layerswitcher-addservices-codsi-filter-btn').addEventListener('click', (e) => {
              this.loadCODSIResults(1);
            });

            document.querySelector('#m-layerswitcher-addservices-codsi-filter-btn').addEventListener('keydown', e => (e.keyCode === 13) && this.loadCODSIResults(1));

            document.querySelector('#m-layerswitcher-addservices-codsi-search-input').addEventListener('keypress', (e) => {
              if (e.keyCode === 13) {
                this.loadCODSIResults(1);
              }
            });

            document.querySelector('#m-layerswitcher-addservices-codsi-clean-btn').addEventListener('click', (e) => {
              document.querySelector('#m-layerswitcher-addservices-codsi-search-input').value = '';
              this.loadCODSIResults(1);
            });
          }
          ...
          document.querySelector('#m-layerswitcher-addservices-search-btn').addEventListener('keydown', (e) => {
            if (e.keyCode === 13) {
              this.filterName = undefined;
              this.readCapabilities(e);
            }
          ...
          document.querySelector('#m-layerswitcher-addservices-search-input').addEventListener('keypress', (e) => {
            if (e.keyCode === 13) {
              this.filterName = undefined;
              this.readCapabilities(e);
            }
          ...
          document.querySelector('div.m-dialog #m-layerswitcher-addservices-search-input').addEventListener('keyup', (e) => {
            const url = document.querySelector('div.m-dialog #m-layerswitcher-addservices-search-input').value.trim();
          ...
          this.accessibilityTab(document.querySelector('.m-layerswitcher-addservices'));
        }, 10);
      }
    }
  ...
  renderInfo(vars) {
    ...
    setTimeout(() => {
      ...
      button.style.width = '75px';
      button.style.backgroundColor = '#71a7d3';
      const elem = document.querySelector('#m-layerswitcher-information .show-more-metadata');
      if (elem !== null) {
        elem.addEventListener('click', () => {
          const block = document.querySelector('#m-layerswitcher-information .more-metadata');
      ...
      this.accessibilityTab(document.querySelector('#m-layerswitcher-information'));
    }, 10);
  }
  ...
  showSuggestions() {
    ...
    document.querySelector('#m-layerswitcher-addservices-suggestions').style.display = 'block';
  }
  ...
  loadCODSIResults(pageNumber) {
    document.querySelector('#m-layerswitcher-addservices-codsi-results').innerHTML = '<p class="m-layerswitcher-loading"><span class="icon-spinner" /></p>';
    const query = document.querySelector('#m-layerswitcher-addservices-codsi-search-input').value.trim();
  ...
  renderCODSIResults(results) {
    document.querySelector('#m-layerswitcher-addservices-codsi-results').innerHTML = '';
    if (results.length > 0) {
      let textResults = '<table><tbody>';
      results.forEach((r) => {
        textResults += `<tr><td><span tabindex="0" class="m-layerswitcher-codsi-result" data-link="${r.url}">${r.title}</span></td></tr>`;
      });

      textResults += '</tbody></table>';
      document.querySelector('#m-layerswitcher-addservices-codsi-results').innerHTML = textResults;
      document.querySelectorAll('#m-layerswitcher-addservices-codsi-results .m-layerswitcher-codsi-result').forEach((elem) => {
      ...
      document.querySelector('#m-layerswitcher-addservices-codsi-results').innerHTML = `<div class="codsi-no-results">${getValue('exception.codsi_no_results')}</div>`;
    }
  }
  ...
  loadSuggestion(evt) {
    ...
    document.querySelector('div.m-dialog #m-layerswitcher-addservices-search-input').value = url;
    if (serviceType === 'WMTS') {
      document.getElementById('m-layerswitcher-addservices-wmts').checked = true;
      document.getElementById('m-layerswitcher-addservices-wms').checked = false;
    } else {
      document.getElementById('m-layerswitcher-addservices-wms').checked = true;
      document.getElementById('m-layerswitcher-addservices-wmts').checked = false;
    }
  ...
  changeLayerConfig(layer, otherStyles) {
    const scroll = document.querySelector('.m-panel.m-plugin-layerswitcher.opened ul.m-layers').scrollTop;
    const styleSelected = document.querySelector('#m-layerswitcher-change-config #m-layerswitcher-style-select').value;
    if (styleSelected !== '') {
      if (layer.type === 'OGCAPIFeatures') {
  ...
  registerCheck(evt) {
    ...
    if (!M.utils.isNullOrEmpty(e.target) && e.target.classList.contains('m-check-layerswitcher-addservices')) {
      const container = document.querySelector('#m-layerswitcher-addservices-results');
      let numNotChecked = container.querySelectorAll('.m-check-layerswitcher-addservices.icon-check').length;
      ...
      if (numNotChecked > 0) {
        this.stateSelectAll = false;
        document.querySelector('#m-layerswitcher-addservices-selectall').classList.remove('icon-check-seleccionado');
        document.querySelector('#m-layerswitcher-addservices-selectall').classList.add('icon-check');
      } else if (numNotChecked === 0) {
        this.stateSelectAll = true;
        document.querySelector('#m-layerswitcher-addservices-selectall').classList.remove('icon-check');
        document.querySelector('#m-layerswitcher-addservices-selectall').classList.add('icon-check-seleccionado');
      }
   ...
   */
  unSelect() {
    const unSelect = document.querySelectorAll('#m-layerswitcher-addservices-results .icon-check-seleccionado');
   ...
   */
  select() {
    const select = document.querySelectorAll('#m-layerswitcher-addservices-results .icon-check');
  ...
  addLayers(evt) {
    evt.preventDefault();
    const layers = [];
    const elmSel = document.querySelectorAll('#m-layerswitcher-addservices-results .icon-check-seleccionado');
    if (elmSel.length === 0) {
  ...
  getProperties(selectValue, summary) {
    document.getElementById('loading-layerswitcher').style.display = 'initial';
    document.getElementById('loading-layerswitcher').style.display = 'none';
    // loading
